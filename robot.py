...
directions = ['forward', 'right', 'back', 'left', 'hey']
...
def add_to_history(command):
    """
    return bot
    Adds the command to the history list of commandsreturn bot
...
def robot_start():
    ...
    and checks if there is an argument passed in the command line"""

    # print(obs.get_obstacles())
    
    global history, position_y, position_x
    
    robot_name = get_robot_name()
    output(robot_name, "Hello kiddo!")

    obstacle.list_of_obstacles = []
    printing_obstacle_position = obstacle.get_obstacles()
    if printing_obstacle_position:
        obstacle.show_position(obstacle.list_of_obstacles)

    if flag_argument:
            obstacles = world.obs.get_obstacles()
            world.show_bot()
            world.plot_position(obstacles)
